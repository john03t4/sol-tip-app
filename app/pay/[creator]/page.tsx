...
import { CheckCircle, Loader2, Twitter, AlertCircle } from "lucide-react";

// USDC Mainnet mint. For devnet testing, use: Gho1n1tH1s1s4fAk3M1ntF0rT3st1ng1nG
...
export default function PayPage() {
  ...
  // Validate creator address on mount
  useEffect(() => {
    try {
      ...
      setError("Invalid creator wallet address. Please check the URL.");
    }
  }, [creator]);
  ...
  const handleTip = async () => {
    ...
    try {
      // Validate + create PublicKey (wrapped in try/catch for safety)
  ...
  const shareOnX = () => {
    const text = encodeURIComponent(
      `Just tipped ${success?.amount} ${success?.token} on SolTip! Support creators directly with crypto. 🐐✨`
    );
  ...
  // Show error state for invalid address
  if (invalidAddress) {
    return (
      <div className="max-w-md mx-auto bg-white rounded-2xl border border-red-200 shadow-lg p-8 text-center">
        <AlertCircle className="w-16 h-16 text-red-500 mx-auto mb-4" />
        <h2 className="text-2xl font-bold text-slate-900 mb-2">Invalid Address</h2>
        <p className="text-slate-600 mb-6">
          The creator address in the URL is not a valid Solana wallet address.
        </p>
        <p className="text-sm text-slate-500 font-mono break-all bg-slate-50 p-3 rounded">
          {creator}
        </p>
        <a
          href="/"
  ...
  if (success) {
    return (
      <div className="max-w-md mx-auto bg-white rounded-2xl border border-slate-200 shadow-lg p-8 text-center">
        ...
        <p className="text-slate-600 mb-6">
          {success.amount} {success.token} sent
        </p>
        <div className="bg-slate-50 rounded-lg p-3 mb-6 text-xs font-mono text-slate-500 break-all">
          Tx: {success.sig}
        </div>
        <button
          onClick={shareOnX}
          className="w-full bg-black text-white py-3 rounded-lg font-medium hover:bg-gray-800 transition flex items-center justify-center gap-2 mb-3"
        >
  ...
  return (
    <div className="max-w-md mx-auto bg-white rounded-2xl border border-slate-200 shadow-lg overflow-hidden">
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-6 text-white text-center">
        ...
        <h1 className="text-2xl font-bold font-mono">{creator?.slice(0, 6)}...{creator?.slice(-4)}</h1>
      </div>
      <div className="p-6 space-y-4">
        {!connected ? (
          ...
          <>
            <div>
              ...
              <div className="flex gap-2">
                ...
                <select
                  ...
                  className="px-4 py-3 rounded-lg border border-slate-300 bg-slate-50 focus:ring-2 focus:ring-indigo-500 outline-none"
                >
            ...
            <button
              onClick={handleTip}
              disabled={loading || !amount || invalidAddress}
            ...
            >
              {loading ? (
                ...
                `Send ${amount || "0"} ${token} Tip`
              )}
            </button>
            <p className="text-xs text-center text-slate-500 mt-2">
              Network fees apply. Transactions are irreversible.
            </p>
          </>
        )}
      </div>
